...
def test_insert_brackets():
    ...
    for i in range(3):
        ...
        print(f"Applied: {complication}")

    # Try to apply an InsertBrackets complication
    brackets_complication = eq_with_sol._try_generate_insert_brackets(random_stream)
    if brackets_complication:
        ...
        print(
            f"Complexity increase: {brackets_complication.complexity(eq_with_sol.get_current_equation())}"
        )
    else:
...
def test_manual_insert_brackets():
    ...
    print(f"Complexity increase: {brackets_comp.complexity(addition)}")
    ...
    print(
        f"Complexity increase with negation: {brackets_comp_neg.complexity(addition)}"
    )
